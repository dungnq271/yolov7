...
def detect(save_img=False):
    ...
    cropped_dir = opt.cropped_dir_name
    save_img = not opt.nosave and not source.endswith('.txt')  # save inference images
    webcam = source.isnumeric() or source.endswith('.txt') or source.lower().startswith(
        ('rtsp://', 'rtmp://', 'http://', 'https://'))

    # Directories
    if osp.exists(opt.project):
        shutil.rmtree(str(opt.project))  # restart

    save_dir = Path(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # increment run
    (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir

    cropped_dir = Path(increment_path(Path(opt.project) / opt.cropped_dir_name, exist_ok=opt.exist_ok))  # increment run
    cropped_dir.mkdir(parents=True, exist_ok=True)  # make dir
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cropped-dir-name', type=str, default='cropped_face_imgs', help='cropped face images storage')
